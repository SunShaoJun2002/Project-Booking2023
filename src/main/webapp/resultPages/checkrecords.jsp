...
<%
	List<Record> record_list = (List<Record>)request.getAttribute("record_list");
	if(record_list == null){
		record_list = new ArrayList<Record>();
		for(int i = 0; i < 10; ++i){
			record_list.add(new Record());
		}
	}
	List<Info> hotel_list = (List<Info>)request.getAttribute("hotel_list");
	if(hotel_list == null){
		hotel_list = new ArrayList<Info>();
		for(int i = 0; i < 10; ++i){
			hotel_list.add(new Info());
		}
	}
	List<List<Room>> bookedRooms = (List<List<Room>>)request.getAttribute("bookedRooms");
	if(bookedRooms == null){
		bookedRooms = new ArrayList<List<Room>>();
		for(int i = 0; i < 10; ++i){
			List<Room> room_list = new ArrayList<Room>();
			for(int j = 0; j < 10; ++j){
				room_list.add(new Room());
			}
			bookedRooms.add(room_list);
		}
	}
	List<List<RecordDetail>> bookedRoomRecords = (List<List<RecordDetail>>)request.getAttribute("bookedRoomRecords");
	if(bookedRoomRecords == null){
		bookedRoomRecords = new ArrayList<List<RecordDetail>>();
		for(int i = 0; i < 10; ++i){
			List<RecordDetail> detail_list = new ArrayList<RecordDetail>();
			for(int j = 0; j < 10; ++j){
				detail_list.add(new RecordDetail());
			}
			bookedRoomRecords.add(detail_list);
		}
	}
%>
			...
			<ul class="bui-tab__nav" data-et-click="  ">
				<li class="bui-tab__item">
					<a aria-current="page" class="bui-tab__link bui-tab__link--selected"
						href="./index.jsp"
						data-et-click="goal:xpb_accommodation goal:xpb_total_clicks" data-ga-track="click|Product Expansion|accommodation|booking (index)">
				...
				<path
					...
					<%
						...
						for(int j = 0; j < record_list.size(); ++j) {
							...
							Info hotel_info = hotel_list.get(j);
							List<Room> room_list = bookedRooms.get(j);
							List<RecordDetail> detail_list = bookedRoomRecords.get(j);
							
							Float totalPrice = 0f;
							for(int k = 0; k < room_list.size(); ++k){
								Integer room_num = 1;
								if(detail_list.get(k) != null){
									room_num = detail_list.get(k).getRoom_num();
								}
								totalPrice += Float.valueOf(room_num) * room_list.get(k).getPrice();
							}
						
							DateFormat dft = new SimpleDateFormat("yyyy-MM-dd");
							String check_in_date = detail_list.get(0).getCheck_in_date();
							if(check_in_date == null){
								check_in_date = "2023-06-14";
							};
							String check_out_date = detail_list.get(0).getCheck_out_date();
					...
					<div id="b_confirmation_body" class="conf_wide_summary conf_align">
						<h2 id="booking_details" class="b_conf_check_details">
							<% out.print("订单: " + record.getRecord_id()); %>
						</h2>
								...
								<div class="conf-summary__main">
									<table class="pb-conf-overview-table">
										<tr>
											<th class="hotel_name larger_hotel_name " colspan="2" scope="row">
												...
												<span class="label-business-trip " title=""><span
														...
														<%
															if(record.getBusiness_or_travel() == 't'){
																out.print("商务旅行");
															}else{
																out.print("自由行");
															}
										...
										<tr>
											...
											<td class="ClickTaleSensitive"><% out.print(record.getRecord_id()); %></td>
									...
									<table class="reservation_details">
										<tbody>
											<tr class="b_conf_overview">
												<th scope="row">预订人姓名</th>
												<td>
													<%
													out.print(record.getBooker_name());
													%>
												</td>
											</tr>
											<tr class="b_conf_overview">
												<th scope="row">预订人电话</th>
												<td>
													<%
											...
											<tr class="b_conf_overview">
												<th scope="row">预订人邮箱</th>
												<td>
													<%
													out.print(record.getEmail());
													%>
												</td>
											</tr>
											<tr class="b_conf_overview">
												<th scope="row">酒店名称</th>
												<td>
													<%
													out.print(hotel_info.getHotel_name());
													%>
											...
											<tr class="b_conf_overview">
												...
												<td>
													...
													for(int i = 0; i < detail_list.size(); ++i) {
														totalRoom_num += detail_list.get(i).getRoom_num();
													}
											...
											<tr class="b_conf_group">
												...
												<td class="conf_room_occupancy_info">
													...
													for(int i = 0; i < room_list.size(); ++i) {
														guest_str += room_list.get(i).getUsable_desc() + ";";
													}
													out.print(guest_str);
													%>
													</p>
												...
												<tbody>
													<% 
														...
														for(int i = 0; i < room_list.size(); ++i) {
															...
															Integer room_num = detail_list.get(i).getRoom_num();
